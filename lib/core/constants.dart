class K {
  static const appName = 'BBML Draft';

  static const supportedLocales = ['tr', 'en', 'ru', 'id', 'fil'];

  static const prefHasSeenOnboarding = 'hasSeenOnboarding';
  static const prefLocaleCode = 'localeCode';

  static const routeOnboarding = '/onboarding';
  static const routeHome = '/home';
  static const routeEnemyPick = '/enemy_pick';
  static const routeEnemyPickResult = '/enemy_pick_result';
  static const routeMyCounters = '/my_counters';
  static const routePopularHeroes = '/popular_heroes';
  static const routeLanguage = '/language_select';
  static const privacyUrl = 'https://massoft.vercel.app/privacypolicy';
  static const appleEulaUrl = 'https://www.apple.com/legal/internet-services/itunes/dev/stdeula/';
}
